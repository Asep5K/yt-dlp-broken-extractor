...
class AnimePaheIE(InfoExtractor, ISO639Utils):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(url, v_id, headers={'user-agent': random_user_agent()})
        return {
            'id': self._html_search_regex(r'<meta\s*name="id"\s*content="(\d+)">', webpage, name='video id', fatal=False, default=v_id),
    ...
    def _get_m3u8_url(self, url):
        v_id =  url.rsplit('/', maxsplit=1)[1]
        webpage = self._download_webpage(url,v_id)
        ...
        if js_decoded:
            ...
            url = self._search_regex(r'const\s*source\s*=\\\'(.*?)\\', js_decoded, name=v_id)
            if url:
                return url
        return False
    ...
    def _build_formats(self, webpage):
        ...
        for u in re.finditer(paterns, webpage, re.DOTALL | re.IGNORECASE):
            ...
            formats.append({
                ...
                'manifest_url': url,
                'language': self.long2short(u.group('aud')),
                'quality': f'{height}p',
                'ext': 'mp4',
                'height': height,
                'http_headers': {
    ...
    def _real_extract(self, url):
        ...
        for page_num in itertools.count(1):
            ...
            for f in data_json['data']:
                entries.append(self.url_result(url=f'{base_url}/{f["session"]}', ie=AnimePaheIE.ie_key(), video_id=f['session'], episode_number=f['episode']))
...
class AnimePaheSearchIE(SearchInfoExtractor):
    ...
    def _search_results(self, query):
        results = self._download_json('https://animepahe.si/api', query, query={'m': 'search', 'q': query})
